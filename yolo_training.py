...
def prepare_yolo_dataset():
    ...
    for src_dir in TRAIN_SRC_DIRS:
        ...
        if not os.path.exists(src_lbl):
             src_lbl = os.path.join(src_dir, 'gt')
    ...
    val_src_lbl = os.path.join(VAL_SRC_DIR, 'labels')
    if not os.path.exists(val_src_lbl):
        val_src_lbl = os.path.join(VAL_SRC_DIR, 'gt')
    ...
    test_src_lbl = os.path.join(TEST_SRC_DIR, 'labels')
    if not os.path.exists(test_src_lbl):
        test_src_lbl = os.path.join(TEST_SRC_DIR, 'gt')
    ...
    final_val_src_lbl = os.path.join(FINAL_VAL_SRC_DIR, 'labels')
    if not os.path.exists(final_val_src_lbl):
        final_val_src_lbl = os.path.join(FINAL_VAL_SRC_DIR, 'gt')
...
def create_data_yaml():
    ...
    data_config = {
        ...
        'final_val': os.path.join('final_val', 'images'),
